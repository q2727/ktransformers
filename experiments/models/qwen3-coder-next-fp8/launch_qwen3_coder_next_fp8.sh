...
MODEL="${MODEL:-/data/qinchong/models/Qwen3-Coder-Next-FP8}"
TP="${TP:-2}"
...
MODEL="${MODEL}" "${SCRIPT_DIR}/verify_qwen3_coder_next_fp8.py" >/dev/null
...
# The two GPUs are attached to different CPU sockets and have no NVLink.
...
if [[ -n "${MAX_MAMBA_CACHE_SIZE:-}" ]]; then
  extra_args+=(--max-mamba-cache-size "${MAX_MAMBA_CACHE_SIZE}")
fi
...
exec python -m sglang.launch_server \
  ...
  --kt-num-gpu-experts "${GPU_EXPERTS:-100}" \
  ...
  --max-total-tokens "${MAX_TOTAL_TOKENS:-256000}" \
