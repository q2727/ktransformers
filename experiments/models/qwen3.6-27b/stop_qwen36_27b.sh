...
if [[ -z "${root_pid}" ]]; then
  root_pid="$(pgrep -u "$(id -u)" -f "[s]glang.launch_server.*Qwen3.6-27B.*--port ${PORT}" | head -1 || true)"
fi
if [[ -z "${root_pid}" ]]; then
  rm -f "${PID_FILE}"
  echo "No matching Qwen3.6-27B server is running."
  exit 0
fi
...
rm -f "${PID_FILE}"
echo "Stopped Qwen3.6-27B server processes."
